...
$registerPage = "../frontend/register.php";
...
if (isset($_POST['username']) && isset($_POST['email']) && isset($_POST['password'])) {
    ...
    if (empty($username) || empty($email) || empty($password)) {
        ...
        if (!mysqli_stmt_prepare($stmt, $check_sql)) {
            header("Location: {$registerPage}?error=Database error during check");
            exit();
        }
        ...
        if (mysqli_num_rows($result) > 0) {
            header("Location: {$registerPage}?error=That email address is already in use");
            exit();
        } else {
            ...
            if (!mysqli_stmt_prepare($stmt2, $insert_sql)) {
                header("Location: {$registerPage}?error=Database error during registration");
                exit();
            }
            ...
            if (mysqli_stmt_execute($stmt2)) {
                ...
                header("Location: {$loginPage}?success=Account created successfully. You can now log in.");
                exit();
            } else {
                header("Location: {$registerPage}?error=An unknown error occurred during registration");
                exit();
            }
        }
    }
